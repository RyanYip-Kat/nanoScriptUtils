...
def load_collapsed_group(all_collapsed_group_file):
    id2pb_dict=dict()
    with open(all_collapsed_group_file,"r") as fin:
        lines=fin.readlines()
        for line in lines:
            d = line.strip().split('\t')
            pbid=d[0]
            readids=d[1].split(",")
            for i,readid in enumerate(readids):
                id2pb_dict[readid]=pbid+"_"+str(i+1)
    fin.close()
    return id2pb_dict
...
def isformMatrix(matrix,tgsbccount,topbc=2500,outfile="isform.matrix.txt"):
    """
...
if __name__=="__main__":
    ...
    collapsed_group=load_collapsed_group(args.group)
    ...
    isformMatrix(matrix,tgsbccount,args.topbc,args.outfile)
