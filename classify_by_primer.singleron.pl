...
while(<IN>){
	chomp;
	@a=();@a=split(/\t/,$_);$slen=@a;
	$m7{$a[0]}{$a[6]}="$a[1]\t$a[2]\t$a[3]\t$a[7]\t-" if($a[1] eq "primer_F" and $a[8]>$a[9] and $a[8]>=21 and $slen==12 and $a[3]>=$min_primerlen);
	$m7{$a[0]}{$a[6]}="$a[1]\t$a[2]\t$a[3]\t$a[7]\t+" if($a[1] eq "primer_F" and $a[8]<$a[9] and $a[9]>=21 and $slen==12 and $a[3]>=$min_primerlen);
	$m7{$a[0]}{$a[6]}="$a[1]\t$a[2]\t$a[3]\t$a[7]\t-" if($a[1] eq "primer_S" and $a[8]>$a[9] and $a[8]>=24 and $slen==12 and $a[3]>=$min_primerlen);
...
while($id=<IN>){
	...
	foreach $k(@key2){
		...
		if($mm>0){
			...
			$barcode=reverse substr($seq,-24);
			$barcode=~tr/ATCGatcg/TAGCtacg/;
			$umi=reverse substr($seq,-69,45);
