...
while(<IN>){
	chomp;
	@a=();@a=split(/\t/,$_);$slen=@a;
	$m7{$a[0]}{$a[6]}="$a[1]\t$a[2]\t$a[3]\t$a[7]\t-" if($a[1] eq "primer_F" and $a[8]>$a[9] and $a[8]>=21 and $slen==12 and $a[3]>=$min_primerlen);  #  primer_F+/- 1 23-2
	$m7{$a[0]}{$a[6]}="$a[1]\t$a[2]\t$a[3]\t$a[7]\t+" if($a[1] eq "primer_F" and $a[8]<$a[9] and $a[9]>=21 and $slen==12 and $a[3]>=$min_primerlen);  
	$m7{$a[0]}{$a[6]}="$a[1]\t$a[2]\t$a[3]\t$a[7]\t-" if($a[1] eq "primer_S" and $a[8]>$a[9] and $a[8]>=24 and $slen==12 and $a[3]>=$min_primerlen);  #  primer_s+/- 1 26-2
	$m7{$a[0]}{$a[6]}="$a[1]\t$a[2]\t$a[3]\t$a[7]\t+" if($a[1] eq "primer_S" and $a[8]<$a[9] and $a[9]>=24 and $slen==12 and $a[3]>=$min_primerlen);
}
close IN;

# $infilename="$ccsfq";
# open(IN,"gzip -dc $ccsfq |") or die ("can not open $ccsfq\n");
open IN,"$ccsfq";
# open FL1,">$outdir/isoseq_flnc.BarcodeUMI.fastq";
open FL2,">$outdir/isoseq_flnc.Transcript.fasta";
...
while($id=<IN>){
	...
	foreach $k(@key2){
		...
		if($mm>0){
			...
			$barcode=reverse substr($seq,-24);  # 24 barcode length
			$barcode=~tr/ATCGatcg/TAGCtacg/;  
			$umi=reverse substr($seq,-69,45);   # 45 UMI length  24+45=69
			$umi=~tr/ATCGatcg/TAGCtacg/;
			
			$bcumi=reverse substr($seq,-69);
			$bcumi=~tr/ATCGatcg/TAGCtacg/;
			# $bcumiqv=reverse substr($qv,-69);
			
			if($tag eq "FL" and $sign{$mm-1}==0){
				$tseq=substr($seq,0,-69);
				# $tqv=substr($qv,0,-69);
				$pa=denovo_poly($tseq);
				...
				$tseq=substr($seq,0,-69);$seqlen=length($tseq);
				$pa=denovo_poly($tseq);
				$polyA="NA";$polyA=substr($tseq,$pa,);
				$Alen=0;$Alen=$polyA=~tr/A/A/;
				if($Alen>=3){
