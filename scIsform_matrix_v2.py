...
def load_collapsed_group(collapsed_group_file,tgsbc,tgsumi,tmapfile=None):
    """
    collapsed_group_file : collapsed group file from collapse_isoforms_by_sam.py
    tgsbc : tgsbc from load_tgsbcumi_fq
    tgsumi : tgsumi from load_tgsbcumi_fq
    tmapfile : flnc.filtered.tmap file from gffcompare
    """
    bc_gene=None
    def load_geneTmap(file):
        bcgene = {}
    ...
    if tmapfile is not None:
        print("loading tmap file")
        bc_gene=load_geneTmap(tmapfile)
    
    print("get matrix and tgs count")
    matrix = defaultdict(lambda: defaultdict(lambda : defaultdict(int)))
    tgsbccount = defaultdict(lambda: defaultdict(int))

    with open(collapsed_group_file,"r") as fin:
        lines=fin.readlines()
        for ii,line in enumerate(lines):
            if (ii+1)%100==0:
                print("reading No.{} line\n".format(str(ii+1)))

            d = line.strip().split('\t')
            a=d[0]
            b=d[1].split(",")
            for i,k in enumerate(b):
                if k in tgsbc and k in tgsumi:
                    bc=tgsbc[k]
                    umi=tgsumi[k]
                    if bc_gene is not None and k in bc_gene:
                        matrix[a+"("+bc_gene[k]+")"][bc][umi]+=1
                    else:
                        matrix[a][bc][umi]+=1
                ...
                if k in tgsbc and k in tgsumi:
                    ...
                    umi=tgsumi[k]
                    if bc_gene is not None and k in bc_gene:
                        isoumi=a+"("+bc_gene[k]+")"+umi
                    else:
                        isoumi=a+umi
                    tgsbccount[bc][isoumi]+=1

    fin.close()
    return matrix,tgsbccount


def load_tmap(tmap,tgsbc,tgsumi,collapsed_group=None,class_code="ckmnjeo"):
    """
    ...
    with open(tmap,'r') as fin:
        ...
        while line:
            ...
            if a[2] in class_code and a[3] in tgsbc and a[3] in tgsumi:
                bc=tgsbc[a[3]]
                umi=tgsumi[a[3]]
                if collapsed_group is not None and a[3] in collapsed_group:
                    matrix[a[0]+"("+collapsed_group[a[3]]+")"][bc][umi]+=1
                else:
                    matrix[a[0]][bc][umi]+=1
            ...
            if a[2] in class_code and a[3] in tgsbc and a[3] in tgsumi:
                ...
                umi=tgsumi[a[3]]
                if collapsed_group is not None and a[3] in collapsed_group:
                   geneumi=a[0]+"("+collapsed_group[a[3]]+")"+umi
                else:
                   geneumi=a[0]+umi
...
def isformMatrix(matrix,tgsbccount,topbc=2500,minUMIcount=1,outfile="isform.matrix.txt"):
    """
    ...
    with open(outfile,"w") as fout:
        ...
        for k1 in key1:
            ...
            exp_sum=0
            for k2 in key2:
                if k1 in matrix and k2 in matrix[k1]:
...
if __name__=="__main__":
    ...
    parser.add_argument('-g','--group',type=str,default=None,help='collapsed group file from collapse_isoforms_by_sam.py')
    parser.add_argument('-o','--outfile',type=str,default="isform.matrix.txt",help='isform.matrix.txt')
    ...
    parser.add_argument('--minUMIcount',type=int,default=2,help="min umi count to bed filtered")
    ...
    print("loading collapsed group file")
    matrix,tgsbccount=load_collapsed_group(args.group,tgsbc,tgsumi,args.tmap)

    #print("loading FLNC Filter Tmap file")
    #matrix,tgsbccount=load_tmap(args.tmap,tgsbc,tgsumi,collapsed_group,class_code=args.class_code)

    print("get Isform Matrix")
    isformMatrix(matrix,tgsbccount,args.topbc,minUMIcount=args.minUMIcount,outfile=args.outfile)
